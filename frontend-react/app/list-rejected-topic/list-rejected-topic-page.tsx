...
export function ListRejectedTopicPageComponent() {
  ...
  // Fetch rejected topics
  useEffect(() => {
    const fetchRejectedTopics = async () => {
      try {
        setLoading(true);
        ...
        const topics = response.data.topicList || response.data || [];
        setRejectedTopics(topics.filter((topic: any) => topic.status === "rejected"));
      } catch (error: any) {
        console.error("Error fetching rejected topics:", error);
        alert("Không thể tải danh sách chủ đề bị từ chối. Vui lòng thử lại!");
        setRejectedTopics([]);
      } finally {
        setLoading(false);
      }
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Bạn có chắc chắn muốn xóa chủ đề này?")) {
      try {
        ...
        // Refresh the list
        const updatedTopics = rejectedTopics.filter(topic => topic.id !== id);
        setRejectedTopics(updatedTopics);
      } catch (error: any) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="bg-white/90 border border-blue-200 rounded-2xl shadow-lg px-8 py-12 text-center">
          ...
          <div className="text-gray-600 text-lg">Chỉ tài khoản <b>Người tạo nội dung</b> hoặc <b>Quản lý</b> mới được xem danh sách chủ đề bị từ chối.<br/>Vui lòng liên hệ quản trị viên nếu bạn cần quyền này.</div>
        </div>
      </div>
    );
  }
            ...
            <div className="relative bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-white/50 overflow-hidden min-w-[800px]">
              ...
              <div className="divide-y divide-blue-100/50">
                {loading ? (
                  ...
                  <div className="px-4 sm:px-8 py-8 text-center">
                    <p className="text-gray-600">Không có chủ đề nào bị từ chối.</p>
                  </div>
                ) : (
                  rejectedTopics.map((topic, index) => (
                    <div key={index} className="px-4 sm:px-8 py-4 sm:py-6 hover:bg-blue-50/30 transition-colors group">
                      <div className="grid grid-cols-6 gap-2 sm:gap-4 items-center text-xs sm:text-sm">
                        <div className="font-bold text-gray-900 truncate">{topic.topicName}</div>
                        <div>
                          <span
                        ...
                        <div className="text-gray-700 font-medium">{topic.isFree ? "Có" : "Không"}</div>
                        <div className="text-gray-700 font-medium">{topic.sortOrder}</div>
                        <div className="text-gray-700 font-medium">{new Date(topic.createdAt).toLocaleDateString('vi-VN')}</div>
...
export default ListRejectedTopicPageComponent 
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white/95 backdrop-blur-sm rounded-3xl p-8 w-full max-w-md text-center shadow-2xl border-4 border-orange-300 relative">
        ...
        {/* Message */}
        <p className="text-lg text-gray-700 mb-4">Bạn cần hoàn thành tất cả các chủ đề nhỏ trước khi làm bài test</p>
        ...
        <div className="bg-orange-50 rounded-xl p-4 mb-6">
          ...
          {completedCount < totalCount && (
            <p className="text-orange-600 text-sm mt-2">Hoàn thành tất cả subtopics để mở khóa bài test</p>
          )}
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white/95 backdrop-blur-sm rounded-3xl p-8 w-full max-w-md text-center shadow-2xl border-4 border-blue-300 relative">
        ...
        {/* Message */}
        <p className="text-lg text-gray-700 mb-6">Bạn cần hoàn thành chủ đề nhỏ trước đó để tiếp tục học</p>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white/95 backdrop-blur-sm rounded-3xl p-8 w-full max-w-md text-center shadow-2xl border-4 border-blue-300 relative">
        ...
        <div className="bg-blue-50 rounded-xl p-4 mb-6">
          ...
          <div className="text-blue-700 font-bold text-lg mb-2">{previousTopicName}</div>
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => {
              ...
              const previousTopicElement = document.querySelector(
                `[data-unit-id="${previousTopicName}"]`,
              ) as HTMLElement
...
export function LearningPath({
  ...
  useEffect(() => {
    const checkSentenceBuilding = async () => {
      ...
      const unitsToCheck = units.filter(
        (unit) => !loadingSentenceBuilding[unit.unitId] && sentenceBuildingInfo[unit.unitId] === undefined,
      )
      ...
      <div className="mb-16">
              ...
              <div>
                ...
                <p className={`text-sm mt-1 ${!isUnitAccessible ? "text-gray-500" : "text-gray-600"}`}>
                  {isUnitAccessible
                    ? unit.description
                    : currentLesson
                      ? "Hoàn thành bài test của chủ đề trước để mở khóa"
                      : unit.lockReason || "Chủ đề này bị khóa"}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="px-4">
          <div className="max-w-2xl mx-auto relative">
            ...
            {isUnitAccessible &&
              sentenceBuildingInfo[unit.unitId] &&
              (() => {
                return (
              ...
              return (
                <div key={lesson.id} className={`relative mb-12 ${getPositionClass(index)}`}>
                  ...
                  <div
                    ...
                    style={{
                      zIndex:
                        selectedLesson === lesson.id.toString() || selectedTest === lesson.id.toString() ? 1000 : 10,
                    }}
                  >
                    {!isUnitAccessible || !isLessonAccessible ? (
                      // Topic hoặc lesson không được truy cập - hiển thị locked state
                      <div className="relative">
                        <button onClick={(e) => handleLessonClick(lesson, e)} className="block relative cursor-pointer">
                          <div className="relative hover:scale-105 transition-transform">
                            {lesson.isTest ? (
                              <div className="relative">
                    ...
                    (() => {
                      ...
                      return (
                        <LessonPopup
                          ...
                          lessonId={Number.parseInt(selectedLesson)}
                  ...
                  {selectedTest === lesson.id.toString() && lesson.isTest && canPopup && selectedTest && unit && (
                    <TestPopup
                      ...
                      testId={Number.parseInt(selectedTest)}
                      topicId={unit.unitId}
                    />
                  )}
                </div>
    ...
    <div className="relative min-h-screen bg-gradient-to-b from-blue-50 to-cyan-50 overflow-hidden pb-20 lg:pb-8 pt-20">
      ...
      >
        <div className="animate-bounce">
          <Image
            src="/images/whale-with-book-new.png" // Updated image source
            alt="Whale character"
            width={180}
            height={180}
            className="object-contain drop-shadow-2xl w-24 h-24 lg:w-36 lg:h-36" // Responsive size
          />
        </div>
      </div>
